...
output_name = '150mM_TEMPOL_short'
...
def run_scans(nScans, power_idx, DNP_data=None):
    ...
    for x in range(nScans):
        ...
        if DNP_data is None:
            ...
            DNP_data['indirect',0]['nScans',x] = data_array
            run_scans_time_list.append(time.time())
...
with power_control() as p:
    ...
    print(retval_thermal)
    p.mw_off()
    print("GOING TO RUN THERMAL SCAN")
    ...
    time.strftime('%Y-%m-%d %H:%M:%S',time.localtime(time.time()))
    print("STARTING ACTUAL ENHANCEMENTS")
    for j, this_dB in enumerate(dB_settings):
        #print("SETTING THIS POWER",this_dB,"(",dB_settings[j-1],powers[j],"W)")
        if j == 0:
            retval = p.dip_lock(9.81,9.83)
            print(retval)
        #retval_Ep = p.dip_lock(9.81,9.83)
        print("THIS IS THE ENHANCEMENT FOR THE %d POWER"%j)    
        p.set_power(this_dB)
        for k in range(10):
            time.sleep(0.5)
            if p.get_power_setting() >= this_dB: break
